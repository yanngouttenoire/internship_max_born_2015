...
int nFieldBirth=100, nVYPerpBirth=200, nVZPrimPerpBirth=100;
...
int main()
{
  ...
  Spectra<state_type> mySpectrum(myPotential, myField, &myIC, angleDetection, binsWidthEnergy);
#endif

#ifdef _OPENMP
  vector<Spectra<state_type> > mySpectra(threadsNbrMax,Spectra<state_type>(myPotential, myField, &myIC, angleDetection, binsWidthEnergy));
#endif
...
/*******We merge all the data binning computed by each thread and write it in a file***********************************/
   ...
   fstream ICFile("icbunch.dat",ios::out);
   
   //We write the data binning in the file "PESFile"
   mySpectrum.writeDataBinning(PESFile,ICFile);
...
/****************************************We build the key of the plot******************************************/
  ...
  myPESPlot.addInstruction("set border 6 back linestyle 100");
  myPESPlot.addInstruction("set grid linestyle 101");
  //We also put the key box in grey
  myPESPlot.addInstruction("set key box ls 100");

  myPESPlot.addInstruction("set xtics scale 0");
  myPESPlot.addInstruction("set format x '' ");
  myPESPlot.addInstruction("set xrange [0:20]");
  myPESPlot.addInstruction("set x2tics 1");
  myPESPlot.addInstruction("set x2label 'Asymptotic energy (eV)'");
  myPESPlot.addInstruction("set ylabel 'Probability (linear scale)'");
  
  //We consider electrons differently depending if they are detected in y>0 or y<0
  std::ostringstream plot1;
  plot1<<"plot 'pes.dat' index 0 using 1:2 w l ls 1 title 'Photo-electrons detected in the upper half-space (y>0), number="<< double(mySpectrum.electronsDetectedUPNbr)/(mySpectrum.electronsDetectedNbr)*100.<<" %', \\";
  ...
  myPESPlot.gnuplot("pes.gnu","pes.eps");
   ...
   /***************************************for the IC*****************************************/ 
    ...
    myICPlot.addInstruction("set multiplot layout 2, 2");
...
//Margins for each row resp. column
  ...
  myICPlot.addInstruction("set_key(x,text) = sprintf(\"set label %f '%s' at graph 0.5,-0.2 center\", x,text)");
  ...
  myICPlot.gnuplot("icbunch.gnu","icbunch.eps");
