...
public:
 ...
 if(whichField=="withField")
      {
	field[0]=+fieldAmpl*pow(sin(M_PI*t/2./pulseDuration),2)*cos(ellipticity)*sin(pulsation*t+phase);
	field[1]=-fieldAmpl*pow(sin(M_PI*t/2./pulseDuration),2)*sin(ellipticity)*cos(pulsation*t+phase);
	field[2]=0.;
      }
...
int main(int /* argc */ , char** /* argv */ )
{
    ...
    //[ integration_class
    // whichField myField=withField;
    harm_osc ho(0.15);
    coulomb m_coulomb("withField");
    steps = integrate( m_coulomb , x , 0.0 , 10.0 , 0.1 );
    //]
